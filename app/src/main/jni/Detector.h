...
void getSkin(cv::Mat &src, cv::Mat &output, const char*rpalm_path, const char*fist_path, const char*right_path)
{
    ...
    //if (!rpalm_cascade.load(NULL))
    if (!rpalm_cascade.load(rpalm_path))
        printf("--(!)Error loading fist\n");
    std::vector<cv::Rect> rpalm;

    //equalizeHist(src,src);
    rpalm_cascade.detectMultiScale(src, rpalm, 1.1, 2, 0 | 2, cv::Size(100, 100));  //80
    //rpalm_cascade.detectMultiScale(src, rpalm, 1.05, 3, 0 | 2, cv::Size(25, 25));
    cv::Rect big_rpalm;
    if (rpalm.size() != 0)
    {
        no_posture_count = 0;
...
//    if (!fist_cascade.load(fist_path))
//        printf("--(!)Error loading fist\n");
//    std::vector<cv::Rect> fists;
//    fist_cascade.detectMultiScale(src, fists, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE, cv::Size(80, 80));
//    cv::Rect big_fist;
//    if (fists.size() != NULL)
//    {
//        no_posture_count = 0;
...
//    if (!right_cascade.load(right_path))
//        printf("--(!)Error loading fist\n");
//    std::vector<cv::Rect> rect_right;
//    right_cascade.detectMultiScale(src, rect_right, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE, cv::Size(80, 80));
//    cv::Rect big_right;

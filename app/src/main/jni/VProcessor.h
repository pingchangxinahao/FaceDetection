...
public:
	// processing method
	virtual void process(cv::Mat &input, cv::Mat &output, const char*rpalm_path, const char*fist_path, const char*right_path) = 0;
};
...
private:
	...
	void(*process)(cv::Mat&, cv::Mat&, const char*rpalm_path, const char*fist_path, const char*right_path);
...
public:
	...
	void setFrameProcessor(void(*frameProcessingCallback)(cv::Mat&, cv::Mat&,const char*,const char*,const char*)) {
	...
	void run(cv::Mat &frame,const char*p1, const char*p2, const char*p3) {
...
//                cv::imshow(windowNameInput, frame);
			...
			if (callIt) {
				...
				{
					printf("--process\n");
					process(frame, output,p1,p2,p3);
				}
				else if (frameProcessor)
				{
					printf("--process 2\n");
					frameProcessor->process(frame, output,p1,p2,p3);
				}
